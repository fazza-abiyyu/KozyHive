  ...
  <div class="w-full lg:ps-64">
    <div class="p-4 sm:p-6 space-y-4 sm:space-y-6">
      <DatatablesDataTable
          ...
          :fields="[
      { label: 'NAMA KOST', key: 'property.name' },
      { label: 'TOTAL HARGA', key: 'totalAmount' },
      { label: 'TANGGAL MASUK', key: 'checkInDate' },
      ...
      { label: 'STATUS PESANAN', key: 'status' },
          ...
          @activeData="(e) => handleActiveData(e)"
          @completedData="(e) => handleCompletedData(e)"
...
const fetchProperties = async () => {
  try {
    ...
    const response: any = await useFetchApi(`/api/admin/bookings?page=${page.value}&limit=${pageSize.value}`);
...
const handleApproveData = async (id: number) => {
  try {
    ...
    await useFetchApi(`/api/bookings/${id}/confirm`, {
      method: 'PATCH',
      headers: {
...
const handleRejectData = async (id: number) => {
  try {
    ...
    await useFetchApi(`/api/bookings/${id}/reject`, {
      method: 'PATCH',
      headers: {
        "Authorization": `Bearer ${token}`
      }
...
const handleActiveData = async (id: number) => {
  try {
    const token = localStorage.getItem("token");
    if (!token) {
      throw new Error("Token tidak ditemukan, harap login ulang.");
    ...
    await useFetchApi(`/api/bookings/${id}/activate`, {
      method: 'PATCH',
      headers: {
        "Authorization": `Bearer ${token}`
      }
...
const handleCompletedData = async (id: number) => {
  try {
    const token = localStorage.getItem("token");
    if (!token) {
      throw new Error("Token tidak ditemukan, harap login ulang.");
    ...
    await useFetchApi(`/api/bookings/${id}/complete`, {
      method: 'PATCH',
      headers: {
        "Authorization": `Bearer ${token}`
      }
