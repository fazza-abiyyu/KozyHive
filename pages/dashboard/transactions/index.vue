  ...
  <div class="w-full lg:ps-64">
    <div class="p-4 sm:p-6 space-y-4 sm:space-y-6">
      <DatatablesDataTable
          ...
          :fields="[
      { label: 'ID', key: 'id' },
      { label: 'NAMA KOST', key: 'booking.property.name' },
      { label: 'JENIS PEMBAYARAN', key: 'paymentMethod' },
...
const fetchProperties = async () => {
  try {
    ...
    const response: any = await useFetchApi(`/api/admin/transactions?page=${page.value}&limit=${pageSize.value}`);
...
const handleChangeFetchData = async (payload: { currentPage: number }) => {
  try {
    isLoading.value = true;
    const response: any = await useFetchApi(`/api/admin/transactions?page=${payload.currentPage}&limit=${pageSize.value}`);
    propertiesData.value = response?.data;
    totalPages.value = response?.meta?.totalPages;
    nextPage.value = response?.meta?.next;
    prevPage.value = response?.meta?.prev;
...
const handleApproveData = async (id: number) => {
  try {
    ...
    await useFetchApi(`/api/transactions/${id}/confirm-payment`, {
      method: 'PATCH',
      headers: {
