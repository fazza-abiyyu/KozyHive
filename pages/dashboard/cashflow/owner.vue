...
const fetchCahsflow = async () => {
  try {
    isLoading.value = true
    const response: any = await useFetchApi(`/api/cashflow/owner?page=${page.value}&limit=${pageSize.value}`);
...
const handleChangeFetchData = async (payload: { currentPage: number }) => {
  try {
    isLoading.value = true;
    const response: any = await useFetchApi(`/api/cashflow/owner?page=${payload.currentPage}&limit=${pageSize.value}`);
    propertiesData.value = response?.data;
    totalPages.value = response?.meta?.totalPages;
    nextPage.value = response?.meta?.next;
    prevPage.value = response?.meta?.prev;
